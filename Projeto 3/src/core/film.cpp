...
namespace rt3 {
    ...
    void Film::add_sample( const Point2f &pixel_coord, ColorXYZ &pixel_color )
    {
        // TODO: add color to the proper location.
        if(pixel_color.e[0] < 1.0){
            pixel_color.e[0] *= 255;
            pixel_color.e[1] *= 255;
            pixel_color.e[2] *= 255;
        }
    ...
    Film * create_film( const ParamSet &ps )
    {
        ...
        if ( result != ps.end() )
        {
            std::vector<float> crop_window = rt3::retrieve<std::vector<float>>(ps, "crop_window");
        }
