...
//Coloquei agora
Camera * make_camera( const std::string &type, const ParamSet& ps_camera, const ParamSet& ps_lookat,  const Film* film )
{
...
void API::world_end( void )
{
    ...
    //-----------------teste
    unique_ptr<Camera> the_camera{ make_camera(render_opt->camera_type, render_opt->camera_ps, 
                                        render_opt->lookat_ps, the_film.get()) };
    ...
    // Recuperando cor do material e vetor de objetos
    vec3<float> color_mat = retrieve<vec3<float>>( render_opt->material_ps, "color");
    
    for(size_t i (0); i < render_opt->objects.size(); i++)
    {
        float radius = retrieve<float>(render_opt->objects[i],  "radius");
        vec3<float> center = retrieve<vec3<float>>(render_opt->objects[i], "center");
        obj = std::make_shared<Sphere>(center, radius);
...
void API::object( const std::vector<ParamSet>& ps_v )
{
