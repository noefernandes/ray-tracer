...
namespace rt3 {
    ...
    class ColorBuffer{
        public:
            ...
            ColorBuffer(const int w, const int h )
            {
                
                data = new unsigned char[h * w * 3];
                std::memset( data, 0, ( h * w * 3 ));
    ...
    class Film {
        public:
            ...
            void add_sample( const Point2f &, ColorXYZ & );
            void write_image() const;
            void turnImage();

            int getFilmWidth(){ return m_color_buffer_ptr->w; }
            int getFilmHeight(){ return m_color_buffer_ptr->h; }
            image_type_e getImageType(){ return image_type; }
