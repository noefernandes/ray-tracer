...
void parse_tags(  tinyxml2::XMLElement *p_element, int level )
{
    ...
    while ( p_element != nullptr )
    {
        ...
        else if( tag_name == "object" )
        {
            ...
            /*else if(retrieve<std::string>(ps, "type") == "trianglemesh")
            { 
            ...
            vector<std::pair<param_type_e, string>> param_list
            {
                ...
                { param_type_e::ARR_REAL    , "indices"}
                { param_type_e::ARR_REAL    , "normals"}
                { param_type_e::ARR_REAL    , "reverse_vertex_order"}
                { param_type_e::STRING    , "backface_cull"}
                
            };
            parse_parameters( p_element, param_list, &ps );
            //std::vector< shared_ptr<Shape>> mesh = create_triangle_mesh_shape(flip_normals, ps );
