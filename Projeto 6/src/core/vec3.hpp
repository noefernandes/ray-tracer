...
public:
    ...
    inline vec3<T> operator+(const float c){
        return vec3<T>(e[0] + c, e[1] + c
            , e[2] + c, e[3] + c);
    }
    ...
    inline vec3<T> operator*(const vec3<T> &v){
        return vec3<T>(e[0] * v.e[0], e[1] * v.e[1], e[2] * v.e[2], 0);
    }
...
inline float dist(const vec3<float> &v1, const vec3<float> &v2){
    float x = v2.e[0] - v1.e[0];
    float y = v2.e[1] - v1.e[1];
    float z = v2.e[2] - v1.e[2];
...
inline float magnitude(const vec3<float> &v){
    float sq_v1 = v.e[0] * v.e[0];
    float sq_v2 = v.e[1] * v.e[1];
    float sq_v3 = v.e[2] * v.e[2];
...
inline float distance(const vec3<float> &v1, const vec3<float> &v2){
