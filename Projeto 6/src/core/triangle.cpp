...
create_triangle_mesh_shape( bool flip_normals, const ParamSet &ps )
{
    ...
    // Retrieve filename.
    string filename = rt3::retrieve<std::string>(ps, "filename"); // Retrieving data associated with 'filename' attrib.
    // Retrieve backface ON/OFF
    string bkf_on_str = rt3::retrieve<std::string>(ps, "backface_cull");
    if ( bkf_on_str == "off" or bkf_on_str == "false" )
        bkfc_on = false;
    // Retrieve Reverse vertex order ON/OFF
    string rvo_str = rt3::retrieve<std::string>(ps, "reverse_vertex_order");
    if ( rvo_str == "on" or rvo_str == "true" )
        reverse_vertex_order = true;
    // Retrieve compute normals flag
    string cn_str = rt3::retrieve<std::string>(ps, "compute_normals");
...
bool load_mesh_data( const std::string & filename, bool rvo, bool cn, bool fn, shared_ptr<TriangleMesh> md )
{
    ...
    // Let us now "convert" or "migrate" the data from tinyobjloader data structure into our mesh data format.
