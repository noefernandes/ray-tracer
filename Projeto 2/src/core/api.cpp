...
void API::world_end( void )
{
    ...
    if ( the_film and the_background )
    {
        ...
        for ( uint j = 0 ; j < h; j++ ) {
            for( uint i = 0 ; i < w ; i++ ) {
                Ray ray1 = the_camera->generate_ray((int) i, (int) j);
                Ray ray2 = the_camera->generate_ray((int) i, (int) j);
                ray1.show();
                ray2.show();
                ...
                the_film->add_sample( Point2f{float(i), float(j)}, color ); // set image buffer at position (i,j), accordingly.
            }
        }
