...
void parse_tags(  tinyxml2::XMLElement *p_element, int level )
{
    ...
    while ( p_element != nullptr )
    {
        ...
        else if( tag_name == "material" )
        {
            ...
            //Atribuir material atual ao smart pointer
            //verificando qual tipo de material criar
            //para atribuir (atualmente só flat)
            // Calling the corresponding API method.
            API::material( ps );
        ...
        else if( tag_name == "object" )
        {
            ...
            if(CSTR_LOWERCASE(p_element->NextSiblingElement()->Value()) != "object"){
            // Calling the corresponding API method.
                API::object( objects_ );
                objects_.clear();
            }
