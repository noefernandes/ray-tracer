...
namespace rt3 {
	...
	PerspectiveCamera* create_perspective_camera(const ParamSet &ps_camera,  const ParamSet ps_lookat, std::shared_ptr<Film> film){
        ...
        float fovy = rt3::retrieve<float>(ps_camera, "fovy");
      	...
      	return new PerspectiveCamera(fovy, lookat, lookfrom, up, film);
	}

	OrthographicCamera* create_orthographic_camera(const ParamSet &ps_camera,  const ParamSet ps_lookat, std::shared_ptr<Film> film){
        ...
        std::vector<float> screen_window = rt3::retrieve<std::vector<float>>(ps_camera, "screen_window");
      	...
      	vec3<float> lookfrom = rt3::retrieve<vec3<float>>(ps_lookat, "look_from");
      	vec3<float> up = rt3::retrieve<vec3<float>>(ps_lookat, "up");
      	return new OrthographicCamera(screen_window, lookat, lookfrom, up, film);
	}
}