...
namespace rt3 {
	class Camera{
		public:
			...
			void setHeightWidth(const int h, const int w){
			    ...
			    //std::cout << this->l << " " << this->r << " " << this->t << " " << this->b << "+++++++++++++++++++++++++++\n";
			}
	...
	class PerspectiveCamera : public Camera {
		public:
			PerspectiveCamera(float fovy, vec3<float> lookat, vec3<float> lookfrom, 
				vec3<float> up, std::shared_ptr<Film> film) : 
										Camera("perspective", fovy, lookat, lookfrom, up)
				{
	...
	class OrthographicCamera : public Camera {
		public:
			...
			OrthographicCamera(std::vector<float> screen_window, vec3<float> lookat, 
				vec3<float> lookfrom, vec3<float> up, std::shared_ptr<Film> film) : 
									Camera("orthographic", screen_window, lookat, lookfrom, up)
			{
