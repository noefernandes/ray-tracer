...
Integrator* make_integrator( const std::string& type, const ParamSet& ps, std::shared_ptr<Camera> camera )
{
    ...
    int depth = retrieve<int>( ps, "depth" );
    ...
    else if(type_integ == "blinn_phong")
    {
        integrator = create_blinn_integrator(ps, camera, depth);
    }
...
void API::world_end( void )
{
    ...
    shared_ptr<Scene> the_scene = std::make_shared<Scene>(the_background, render_opt->objects, render_opt->lights, render_opt->ambient);

    //std::cout << "*****************************************> " << render_opt->lights.size() << "\n";
    ...
    //Apagando vetores
    render_opt->ambient = nullptr;
    render_opt->objects.clear();
    render_opt->named_materials.clear();
...
void API::lights( const std::vector<std::shared_ptr<Light>> lights_ )
{
    std::cout << ">>> Inside API::lights()\n";
    VERIFY_WORLD_BLOCK("API::lights");

    // retrieve type from ps.
    //string type = retrieve( ps, "type", string{"unknown"} );
    //render_opt->material_type = type;
    // Store current material object.
    render_opt->lights.insert(render_opt->lights.end(), lights_.begin(), lights_.end());
}
 
void API::setAmbientLight(AmbientLight* ambient)
{
    render_opt->ambient = ambient;
}
...
void API::make_named_materials( const std::vector<std::shared_ptr<BlinnMaterial>> named_materials_ )
{
    std::cout << ">>> Inside API::named_materials()\n";
    VERIFY_WORLD_BLOCK("API::named_materials");

    render_opt->named_materials = named_materials_;
}
